...
import { shopCategories } from 'apps/vendor-ui/src/utils/categories'
import axios from 'axios'
import React from 'react'
import { useForm } from 'react-hook-form'

function CreateShop({vendorId,
  setActiveStep,
} : {
  vendorId: string
 ...
 const {register,
  ...
  const onSubmit = async (data: any) => {
    const shopData = {...data, vendorId}
    shopCreateMutation.mutate(shopData)
}

const countWards = (text: string) => text.trim().split(/\s+/).length
    ...
    <div>
        <form onSubmit={handleSubmit(onSubmit)}>
                ...
                {...register("bio", {
                  ...
                  validate: (value) =>
                    countWards(value) <= 100 || "Bio cannot exceed 100 words"
                }
              )
              }/>
               {errors.bio && (
            ...
            <button 
            type = {"submit"}
            className='w-full text-lg bg-orange-600 text-white py-2 rounded-lg mt-4'>
              Create
            </button>
